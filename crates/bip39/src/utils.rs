...
use crate::{Error, Language, Result, WordCount};
...
/// assert_ne!(seed, seed_with_pass); // Different passphrases produce different seeds
/// ```
pub fn phrase_to_seed_in_language(
    phrase: &str,
    passphrase: &str,
    language: Language,
) -> Result<[u8; 64]> {
    ...
    let upstream_language = language.to_upstream();
    let mnemonic = bip39_upstream::Mnemonic::parse_in_normalized(upstream_language, phrase)
...
pub fn generate_mnemonic_in_language(word_count: WordCount, language: Language) -> Result<String> {
    ...
    let mnemonic =
        bip39_upstream::Mnemonic::from_entropy_in(upstream_language, &entropy).map_err(|_| {
...
mod tests {
    ...
    fn test_validate_phrase_valid_12_words() {
        ...
        assert!(
            result.is_ok(),
            "Valid 12-word phrase should pass validation"
        );
    }
    ...
    fn test_validate_phrase_valid_24_words() {
        ...
        assert!(
            result.is_ok(),
            "Valid 24-word phrase should pass validation"
        );
    }
    ...
    fn test_validate_phrase_valid_15_words() {
        ...
        assert!(
            result.is_ok(),
            "Generated valid 15-word phrase should pass validation: {}",
            phrase
        );
    }
    ...
    fn test_validate_phrase_valid_18_words() {
        ...
        assert!(
            result.is_ok(),
            "Generated valid 18-word phrase should pass validation: {}",
            phrase
        );
    }
    ...
    fn test_validate_phrase_valid_21_words() {
        ...
        assert!(
            result.is_ok(),
            "Generated valid 21-word phrase should pass validation: {}",
            phrase
        );
    }
    ...
    fn test_validate_phrase_extra_whitespace() {
        ...
        assert!(
            result.is_ok(),
            "Phrase with extra whitespace should be normalized and pass validation"
        );
    }
    ...
    fn test_validate_phrase_empty_string() {
        ...
        match result.unwrap_err() {
            Error::InvalidMnemonic { reason } => {
                assert!(
                    reason.contains("empty") || reason.contains("Empty"),
                    "Should indicate empty phrase: {}",
                    reason
                );
            }
    ...
    fn test_validate_phrase_invalid_word_first_position() {
        ...
        assert!(
            result.is_err(),
            "Phrase with invalid first word should fail validation"
        );
    ...
    fn test_validate_phrase_invalid_word_middle_position() {
        ...
        assert!(
            result.is_err(),
            "Phrase with invalid middle word should fail validation"
        );
    ...
    fn test_validate_phrase_invalid_word_last_position() {
        ...
        assert!(
            result.is_err(),
            "Phrase with invalid last word should fail validation"
        );
    }
    ...
    fn test_validate_phrase_multiple_invalid_words() {
        ...
        assert!(
            result.is_err(),
            "Phrase with multiple invalid words should fail validation"
        );
    ...
    fn test_validate_phrase_invalid_checksum_12_words() {
        ...
        assert!(
            result.is_err(),
            "Phrase with invalid checksum should fail validation"
        );
    ...
    fn test_validate_phrase_invalid_checksum_24_words() {
        ...
        assert!(
            result.is_err(),
            "24-word phrase with invalid checksum should fail validation"
        );
    }
    ...
    fn test_validate_phrase_numbers_as_words() {
        let phrase =
            "123 456 789 abandon abandon abandon abandon abandon abandon abandon abandon about";
        let result = validate_phrase(phrase);
        assert!(
            result.is_err(),
            "Phrase with numbers should fail validation"
        );
    ...
    fn test_validate_phrase_special_characters() {
        ...
        assert!(
            result.is_err(),
            "Phrase with special characters should fail validation"
        );
    }
    ...
    fn test_validate_phrase_unicode_characters() {
        ...
        assert!(
            result.is_err(),
            "Phrase with unicode characters should fail validation"
        );
    }
    ...
    fn test_validate_phrase_all_valid_word_counts() {
        ...
        for (expected_count, phrase) in test_cases.iter() {
            ...
            assert_eq!(
                words.len(),
                *expected_count,
                "Test phrase should have correct word count"
            );

            // Validate that our WordCount enum accepts this count
            assert!(
                WordCount::from_word_count(*expected_count).is_ok(),
                "WordCount should accept {} words",
                expected_count
            );
        }
    }
    ...
    fn test_validate_phrase_in_language_english() {
        ...
        assert!(
            result.is_ok(),
            "Valid English phrase should pass validation"
        );
    }
    ...
    fn test_validate_phrase_in_language_different_languages() {
        ...
        assert!(
            result.is_ok(),
            "English phrase should pass English validation"
        );
        ...
        assert!(
            result.is_err(),
            "English phrase should fail Japanese validation"
        );
        ...
        match result.unwrap_err() {
            Error::InvalidWord { word, position } => {
                assert_eq!(
                    word, "abandon",
                    "First English word should be invalid in Japanese"
                );
        ...
        assert!(
            ...
            "Invalid phrase should fail in any language"
        );
    }
    ...
    fn test_language_integration() {
        ...
        for &language in Language::all_variants() {
            ...
            assert!(
                result.is_err(),
                "Invalid phrase should fail in {} language",
                language.name()
            );
        }
    }
    ...
    fn test_phrase_to_seed_without_passphrase() {
        ...
        assert_eq!(
            &seed[..],
            &expected_seed[..],
            "Seed should match BIP39 test vector"
        );
    }
    ...
    fn test_phrase_to_seed_with_passphrase() {
        ...
        assert_eq!(
            &seed[..],
            &expected_seed[..],
            "Seed with passphrase should match BIP39 test vector"
        );
    }
    ...
    fn test_phrase_to_seed_passphrase_affects_seed() {
        ...
        assert_ne!(
            seed1, seed2,
            "Empty passphrase should produce different seed than 'password'"
        );
        assert_ne!(
            seed2, seed3,
        ...
        assert_ne!(
            seed1, seed3,
            "Empty and 'different' should produce different seeds"
        );
    }
    ...
    fn test_phrase_to_seed_24_word_phrase() {
        ...
        assert_eq!(
            seed.len(),
            64,
            "24-word phrase should also produce 64-byte seed"
        );
        ...
        assert_ne!(
            seed, seed_12_word,
            "24-word and 12-word phrases should produce different seeds"
        );
    }
    ...
    fn test_phrase_to_seed_whitespace_in_passphrase() {
        ...
        assert_ne!(
            seed2, seed3,
            "Different whitespace patterns should produce different seeds"
        );
    }
    ...
    fn test_phrase_to_seed_case_sensitive_passphrase() {
        ...
        assert_ne!(
            seed1, seed2,
            "Different case should produce different seeds"
        );
        assert_ne!(
            seed2, seed3,
            "Different case should produce different seeds"
    ...
    fn test_phrase_to_seed_all_word_counts() {
        ...
        for (word_count, entropy) in test_cases.iter() {
            ...
            assert_eq!(
                seed.len(),
                64,
                "{}-word phrase should produce 64-byte seed",
                word_count
            );
        }
    }
    ...
    fn test_generate_mnemonic_12_words() {
        ...
        assert!(
            validate_phrase(&mnemonic).is_ok(),
    ...
    fn test_generate_mnemonic_15_words() {
        ...
        assert!(
            validate_phrase(&mnemonic).is_ok(),
            "Generated 15-word mnemonic should be valid"
        );
    }
    ...
    fn test_generate_mnemonic_18_words() {
        ...
        assert!(
            validate_phrase(&mnemonic).is_ok(),
            "Generated 18-word mnemonic should be valid"
        );
    }
    ...
    fn test_generate_mnemonic_21_words() {
        ...
        assert!(
            validate_phrase(&mnemonic).is_ok(),
            "Generated 21-word mnemonic should be valid"
        );
    }
    ...
    fn test_generate_mnemonic_24_words() {
        ...
        assert!(
            validate_phrase(&mnemonic).is_ok(),
            "Generated 24-word mnemonic should be valid"
        );
    }
    ...
    fn test_generate_mnemonic_can_generate_seed() {
        ...
        assert_eq!(
            seed.len(),
            64,
            "Generated mnemonic should produce valid seed"
        );
    }
    ...
    fn test_generate_mnemonic_in_language_english() {
        ...
        assert_eq!(words.len(), 12, "Generated mnemonic should have 12 words");
        assert!(
            validate_phrase_in_language(&mnemonic, Language::English).is_ok(),
    ...
    fn test_generate_mnemonic_in_language_japanese() {
        ...
        let mnemonic =
            generate_mnemonic_in_language(WordCount::Twelve, Language::Japanese).unwrap();

        let words: Vec<&str> = mnemonic.split_whitespace().collect();
        assert_eq!(
            words.len(),
            12,
            "Generated Japanese mnemonic should have 12 words"
        );
        assert!(
            validate_phrase_in_language(&mnemonic, Language::Japanese).is_ok(),
    ...
    fn test_generate_mnemonic_all_word_counts() {
        ...
        for &word_count in word_counts {
            ...
            assert_eq!(
                ...
                "Generated mnemonic should have {} words",
                word_count.word_count()
            );
            assert!(
                validate_phrase(&mnemonic).is_ok(),
                "Generated {}-word mnemonic should be valid",
                word_count.word_count()
            );
        }
    }
    ...
    fn test_generate_mnemonic_all_languages() {
        ...
        for &language in languages {
            ...
            assert!(
                validate_phrase_in_language(&mnemonic, language).is_ok(),
                "Generated {} mnemonic should be valid",
                language.name()
            );
        }
    ...
    fn test_generate_mnemonic_deterministic_seed() {
        ...
        assert_eq!(
            seed1, seed2,
            "Same mnemonic should always produce same seed"
        );
    }
    ...
    fn test_generate_mnemonic_entropy_strength() {
        ...
        assert!(
            mnemonic_12.len() < mnemonic_24.len(),
