...
fn scenario_address_gap_limit_scanning() {
    ...
    let addresses = account
        .derive_address_range(Chain::External, 0, gap_limit)
        .unwrap();
...
fn scenario_ethereum_multi_account() {
    ...
    for i in 0..5 {
        let account = wallet
            .get_account(Purpose::BIP44, CoinType::Ethereum, i)
            .unwrap();
    ...
    for i in 0..eth_addresses.len() {
        for j in (i + 1)..eth_addresses.len() {
            assert_ne!(
                eth_addresses[i].private_key(),
                eth_addresses[j].private_key()
            );
        }
    }
}
...
fn scenario_hardware_wallet_paths() {
    ...
    let ledger_key = master_key
        .derive_path(&ledger_btc.to_derivation_path())
        .unwrap();

    // Ledger Ethereum path: m/44'/60'/0'/0/0
    let ledger_eth =
        Bip44Path::new(Purpose::BIP44, CoinType::Ethereum, 0, Chain::External, 0).unwrap();
    let ledger_eth_key = master_key
        .derive_path(&ledger_eth.to_derivation_path())
        .unwrap();
...
fn scenario_multi_language_support() {
    ...
    let wallet_en =
        Wallet::from_mnemonic(mnemonic_en, "", Language::English, Network::BitcoinMainnet).unwrap();
...
fn scenario_high_volume_addresses() {
    ...
    let addresses = account
        .derive_address_range(Chain::External, 0, 1000)
        .unwrap();
