...
use khodpay_bip44::{
    Account, Bip44Path, Chain, CoinType, Purpose, Wallet, WalletBuilder,
};
...
fn test_complete_workflow_mnemonic_to_addresses() {
    ...
    let account = Account::from_extended_key(account_key.clone(), Purpose::BIP44, CoinType::Bitcoin, 0);
...
fn test_wallet_from_mnemonic_integration() {
    ...
    let mut wallet = Wallet::from_mnemonic(mnemonic, "", Language::English, Network::BitcoinMainnet).unwrap();
...
fn test_different_bip_purposes() {
    ...
    let addr49 = {
        let account = wallet.get_account(Purpose::BIP49, CoinType::Bitcoin, 0).unwrap();
        account.derive_external(0).unwrap()
    };
    
    let addr84 = {
        let account = wallet.get_account(Purpose::BIP84, CoinType::Bitcoin, 0).unwrap();
        account.derive_external(0).unwrap()
    };
    
    let addr86 = {
        let account = wallet.get_account(Purpose::BIP86, CoinType::Bitcoin, 0).unwrap();
        account.derive_external(0).unwrap()
    };
...
fn test_seed_vs_mnemonic_equivalence() {
    ...
    let wallet1 = Wallet::from_english_mnemonic(mnemonic_phrase, "", Network::BitcoinMainnet).unwrap();
    ...
    assert_eq!(wallet1.master_key().private_key(), wallet2.master_key().private_key());
    assert_eq!(wallet1.master_key().chain_code(), wallet2.master_key().chain_code());
}
...
fn test_password_protected_mnemonic() {
    ...
    let wallet_with_pass = Wallet::from_english_mnemonic(mnemonic, password, Network::BitcoinMainnet).unwrap();
    
    // Create wallet without password
    let wallet_no_pass = Wallet::from_english_mnemonic(mnemonic, "", Network::BitcoinMainnet).unwrap();
...
fn test_multi_coin_wallet() {
    ...
    let btc_addr = {
        let btc = wallet.get_account(Purpose::BIP44, CoinType::Bitcoin, 0).unwrap();
        btc.derive_external(0).unwrap()
    };
    
    let eth_addr = {
        let eth = wallet.get_account(Purpose::BIP44, CoinType::Ethereum, 0).unwrap();
        eth.derive_external(0).unwrap()
    };
    
    let ltc_addr = {
        let ltc = wallet.get_account(Purpose::BIP44, CoinType::Litecoin, 0).unwrap();
...
fn test_address_batch_derivation() {
    ...
    let addresses = account.derive_address_range(Chain::External, 0, 10).unwrap();
